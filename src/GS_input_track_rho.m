function GS_input_track_rho(N,filename,parahom,p)
    % problem parameters
    DA = p.DA;  DS = p.DS;    
    mu = p.mu;  rho = p.rho;
    a = p.xl;   b = p.xr;
...
%     end
%     if userhom
%         variable=[variable,char(vSl(N+1))];
%     else
%         variable=[variable,char(vSl(N+1))];
%     end
...
%             fprintf(fid,ftmp);
    ...
    %         fprintf(fid,ftmp);
            ...
            % interior Al and Sl equations
%             tmp=(sym('DA')*(vAl(k+1)+vAl(k-1)-2*vAl(k))/sym('h')^2-vAl(k))*sym('mu')+(vSl(k+1)+vSl(k-1)-2*vSl(k))/sym('h')^2;
%             
%             ftmp=['fAl',num2str(k), ' = ',char(tmp),';\n'];
%             fprintf(fid,ftmp);
%             
%             tmp=sym('DS')*(vSl(k+1)+vSl(k-1)-2*vSl(k))/sym('h')^2-sym('mu')*(vA(k)^2*vSl(k)+2*vA(k)*vS(k)*vAl(k));
%             ftmp=['fSl',num2str(k), ' = ',char(tmp),';\n'];
%             fprintf(fid,ftmp);
