function GS_input_bdry(N,filename,parahom,p)
    % problem parameters
    DA = p.DA;  DS = p.DS;  mu = p.mu;
    a = p.xl;   b = p.xr;
    h=(b-a)/N;
...
%     end
    ...
    if ~parahom
        ...
        fprintf(fid,'variable_group rho;\n\n');
    else
        fprintf(fid,[variable,char(vSl(N+1)),';\n']);
    end
    ...
    fprintf(fid,[ff,';\n']);
    fprintf(fid,'constant mu,h,DA,DS;\n');
...
%             fprintf(fid,ftmp);
            ...
            tmp=sym('DA')*(vAl(k+1)+vAl(k-1)-2*vAl(k))/sym('h')^2-vAl(k)...
                *(sym('mu')+sym('rho'))+sym('DS')*(vSl(k+1)+vSl(k-1)...
                -2*vSl(k))/sym('h')^2-sym('rho')*vSl(k);
            ftmp=['fAl',num2str(k), ' = ',char(tmp),';\n'];
            fprintf(fid,ftmp);
            tmp=sym('DS')*(vSl(k+1)+vSl(k-1)-2*vSl(k))/sym('h')^2-...
                vA(k)^2*vSl(k)-2*vA(k)*vS(k)*vAl(k)-sym('rho')*vSl(k);
            ftmp=['fSl',num2str(k), ' = ',char(tmp),';\n'];
            fprintf(fid,ftmp);
