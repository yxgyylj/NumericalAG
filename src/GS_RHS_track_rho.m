...
%
%   Start to track rho (treating mu as time variable). ode solver shall be
% used for this function
%
%   Written by Xige Yang, Ohio State University
%=========================================================================

function out = GS_RHS_track_rho(mu,x,p,flag)
    ...
    for j = 2:Nx-1
        ...
        out(j) = out(j) + A(j)^2*S(j) - (mu + rho_var)*A(j);   % reaction
        out(j+Nx) = (p.DS*(S(j-1) - 2*S(j) + S(j+1)))/h^2;      % diffusion
        out(j+Nx) = out(j+Nx) - A(j)^2*S(j) + rho_var*(S(j) - 1);  % reaction
        
        % diffusion and reaction for first order perturbation terms
        out(j+2*Nx) = (p.DA*(Al(j-1) - 2*Al(j) + Al(j+1)))/h^2;      
        out(j+2*Nx) = out(j+Nx) + A(j)^2*Sl(j) + 2*S(j)*A(j)*Al(j) ...
             - (mu + rho_var)*Al(j);
        out(j+3*Nx) = (p.DS*(Sl(j-1) - 2*Sl(j) + Sl(j+1)))/h^2;
        out(j+3*Nx) = out(j+Nx) - A(j)^2*Sl(j) - 2*S(j)*A(j)*Al(j) - rho_var*Sl(j);
    end
